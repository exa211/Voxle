...
void Pipeline::createGraphicsPipeline(VkDescriptorSetLayout& descriptor) {
  ...
  if(vkCreatePipelineLayout(E_Data::i()->vkInstWrapper.device, &pipelineLayoutInfo, nullptr, &layout) != VK_SUCCESS) LOG::fatal("Could not create VkPipelineLayout in GraphicsPipeline.cpp");
  ...
  if(vkCreateGraphicsPipelines(E_Data::i()->vkInstWrapper.device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &E_Data::i()->vkInstWrapper.pipeline) != VK_SUCCESS)
...
void Pipeline::createFramebuffers() {
  ...
  for(size_t i = 0; i < imageViews.size(); i++) {
    ...
    if(vkCreateFramebuffer(E_Data::i()->vkInstWrapper.device, &framebufferInfo, nullptr, &buffers[i]) != VK_SUCCESS) LOG::fatal("Error while creating Framebuffers");
    ++frameBufferCount;
  }
...
void Pipeline::createCommandPool() {
  QueueFamilyIndices queueFamilyIndices = QueueHelper::findQueueFamilies(E_Data::i()->vkInstWrapper.physicalDevice, E_Data::i()->vkInstWrapper.surface);
  VkCommandPoolCreateInfo poolInfo{};
  poolInfo.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
  poolInfo.flags = VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT;
  poolInfo.queueFamilyIndex = queueFamilyIndices.graphicsFamily.value();
  if(vkCreateCommandPool(E_Data::i()->vkInstWrapper.device, &poolInfo, nullptr, &E_Data::i()->vkInstWrapper.commandPool) != VK_SUCCESS)
    LOG::fatal("Could not create VkCommandPool");
  LOG::info("Created VkCommandPool");

  // IMMEDIATE UPLOAD POOL

  if(vkCreateCommandPool(E_Data::i()->vkInstWrapper.device, &poolInfo, nullptr, &E_Data::i()->vkInstWrapper.immediateUploadPool) != VK_SUCCESS)
    LOG::fatal("Could not create VkCommandPool");
...
void Pipeline::recordCommandBuffer(Camera& cam, VkCommandBuffer commandBuffer, uint32_t imageIndex) {
  ...
  for(Mesh& m : SceneManager::i()->curScene.meshesInScene) {
    ...
    vkCmdPushConstants(commandBuffer, E_Data::i()->vkInstWrapper.pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(MeshPushConstant), &constant);
    ...
    // Bind UBO
    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, vki.pipelineLayout, 0, 1, vki.descriptorSets.data(), 0, nullptr);
